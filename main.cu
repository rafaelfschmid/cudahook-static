...
void callcudahook(int n, int streams) {
  if (!(hMyLib = MyLoadLib("/home/rafael/cuda-workspace/cudahook-master/libcudahook.so"))) { /*error*/ }
...
int main(int argc, char **argv) {
	...
	sem_t *sem_1 = sem_open("semaforo1", O_CREAT, S_IRUSR | S_IWUSR, 0);
	//sem_t *sem_2 = sem_open("semaforo2", O_CREAT, S_IRUSR | S_IWUSR, 0);
	//sem_t *sem_3 = sem_open("semaforo3", O_CREAT, S_IRUSR | S_IWUSR, 0);

	//sem_init("semaforo1", 0);
	//sem_init("semaforo1", 0);

	// Semaphore
	//sem_t *sem_1 = segment.construct<sem_t>("sem_1")();
	sem_init(sem_1, 1, 1);
	// Index of threads
	...
	vec.push_back(std::async(std::launch::async,callcudahook, 2, 2));
...
/*	printf("começoooouuuu\n");
	...
	/*segment.destroy<int>("Index");
	segment.destroy<SharedMap>("Kernels");
	segment.destroy<ConditionVariable>("ConditionVariable");
	segment.destroy<Mutex>("Mutex");*/
